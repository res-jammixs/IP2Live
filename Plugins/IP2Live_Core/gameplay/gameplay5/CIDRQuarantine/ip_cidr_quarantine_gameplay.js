 ...
 * Stage 3 Level 2+ path connector quarantine puzzle.
 */
...
class IP2LiveCIDRQuarantineGameplayScreen extends Scene.Base {
    ...
    _generateProblem(spec) {
        ...
        for (let attempt = 0; attempt < 80; attempt++) {
            ...
            if (this._sameTile(start, cursor) || this._manhattan(start, cursor) < difficulty.minEndpointDistance) continue;
    ...
    _showDiagnostic(result) {
        ...
        if (IP2Live.GameManager && typeof IP2Live.GameManager.handleGameplayMistake === 'function') {
            IP2Live.GameManager.handleGameplayMistake('ip_cidr_quarantine', {
                mapId: this.options.mapId || (this.options.spec && this.options.spec.mapId) || 12,
    ...
    _finishSuccess(result) {
        ...
        if (sharedPopup && typeof sharedPopup.begin === 'function' && sharedPopup.begin(this, {
            gameplayId: 'ip_cidr_quarantine',
            label: this.options.questLabel || (this.options.spec && this.options.spec.label) || 'CIDR quarantine route secured',
            footer: 'QUARANTINE PATH VERIFIED  //  PROGRESS SECURED',
            durationMs: 950,
            result: payload,
            onComplete: complete,
        })) {
    ...
    _applyVirusOverrunFailure() {
        ...
        if (this.tutorialMode) {
            ...
            this._resetTutorialAttemptAfterFailure();
    ...
    _resetTutorialAttemptAfterFailure() {
        ...
        this.trace = null;
        this._initVirusSpread();
        this.statusText = 'Attempt reset. Rebuild the connector with the same clues.';
        this.statusTone = 'idle';
    }
    ...
    _drawVirusMeter(ctx, m, x, y, w, h) {
        ...
        ctx.font = 'bold ' + Math.round(11 * m.sY) + 'px monospace';
        ctx.textAlign = 'left';
        ctx.fillText('Virus pressure ' + this.problem.viruses.length + '/256', x, y + h + 16 * m.sY);
        ctx.restore();
    }
    ...
    _randomCIDRClass(difficulty) {
        ...
        return (viable.length ? viable : classes)[this._randomInt(0, (viable.length ? viable : classes).length - 1)];
    }
